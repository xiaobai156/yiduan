...
from duan_app.parsing.engine import has_result_keyword, is_strict_segment, iter_issue_segments, iter_search_texts, score_segment, table_signature_count, title_in_segment, values_in_segment
...
from duan_app.text_utils import compact_line, html_to_text, normalize_text, strip_hidden_html_blocks
...
def site_section_anchor_terms(site: Site) -> tuple[str, ...]:
    return (site.name, *SITE_SECTION_ANCHOR_ALIASES.get(site.name, ()))

def has_site_section_result_marker(text: str) -> bool:
    normalized = normalize_text(text).replace(" ", "")
    return has_result_keyword(normalized) or "绝杀㊣一段" in normalized

def site_section_title_issues(documents: list[str], site: Site) -> set[int]:
    ...
    for document in documents:
        ...
        for anchor_term in site_section_anchor_terms(site):
            ...
            if selected is not None:
                issues.add(int(selected.group(1).translate(FULLWIDTH_DIGITS)))
    return issues
...
def collect_three_landlords_candidates(
    ...
    for document_index, document in enumerate(documents):
        ...
        marker_count += count
        if count:
            authorities.append((document_index, text))
    if marker_count != 1 or len(authorities) != 1:
        return []

    document_index, text = authorities[0]
    start = text.find(THREE_LANDLORDS_MARKER) + len(THREE_LANDLORDS_MARKER)
    end = text.find(THREE_LANDLORDS_BOUNDARY, start)
...
def collect_candidates(
    ...
    *,
    ...
    allow_repeated_duan = site.name in ALLOW_REPEATED_DUAN_SITE_NAMES
    allow_multi_duan = site.name in MULTI_DUAN_SITE_NAMES
    title_issues = site_section_title_issues(documents, site)
        ...
        for text_index, text in enumerate(document_texts):
            ...
            for search_text, base_position in search_texts:
                ...
                for issue, issue_text, segment in iter_issue_segments(search_text, issue_filter):
                    ...
                    for value in values:
                        key = (issue, value, candidate_title, snippet)
                        if key in seen:
                            continue
                        seen.add(key)
                        candidates.append(
